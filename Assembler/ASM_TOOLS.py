...
def read_vec(CODE_LINE):
    ...
    CODE_LINE = CODE_LINE[1:-1] #strip away brakets or quotes
    VALS = []
    if (line_type == "String"):
        ...
        for item in CODE_LINE:
            VALS.append(read_int_lit(CODE_LINE)) #bleep it only intergers in the code line no reason for vars or addresses to be in a vector
    return VALS            
