...
class jz_101_assembler:
	...
	def __init__(self):
		...
		self.TWINS = ("LDA","STR","INC","DEC",
		  "JMP","CAL","JMP","JME",
		  "JML","JMG","JMC")
		self.NOINS = ("NOP","RTN","HTI","HCF","HAD","HDB")
	...
	def get_val(self,Parm,addr):
		if Parm in self.symbol_table.keys():
			return self.get_s_val(Parm,addr)
		try:
			return self.get_int(Parm)
		except Assembly_Error:
			raise Assembly_Error("unknown value",Parm)
			
	def get_s_val(self,parm,addr):
		#this is a temp messure to calculate the 
		#offset of a address because where branching
		#now insted of Jumping
		if (parm[0] == "@"):
			parm_addr = self.symbol_table[parm]
			if (parm_addr > addr):
				#if our addr is a head then we simply jump that far ahed
				return parm_addr - addr
			else:
				#otherwise return the negitive signed distance back to our parm
				return (addr - parm_addr) + 0x8000 
		else:
	...
	def read_ins(self,line,addr):
		...
		p1 = self.get_val(sections[1],addr) << 4 #getoprand 1
		p2 = self.get_val(sections[2],addr)
		opcode += p1 + p2
		self.values.append([addr,opcode,line])
		...
		self.values.append([addr + 1,self.get_val(sections[3],addr),""]) 

	def read_var(self,line):
		sections = self.split_comma(line)[1:] #get everthing but init
		for sec in sections:
			self.values.append([None,self.get_val(sec,0),None]) #append values
...
if __name__ == "__main__":
	...
	def read_assembly_file(fname):
		asm_file = open(fname,'r')
		asm_string = asm_file.read()
		asm_file.close()
		return asm_string
	
	def write_hex_file(fname,assembler):
		hex_file = open(fname,'w')
		hex_file.write(assembler.get_output())
		hex_file.close()
		
	def write_secondary_output(fname,assembler):
		so_file = open(fname,'w')
		so_file.write(assembler.get_secondary_output())
		so_file.close()
		
	base = ""
	asm_fname = input("assembly file: ")
	if asm_fname[-2:] == ".s":
		asm_fname = asm_fname[:-2] #if extention is used remove the extention
	asm_code = read_assembly_file(asm_fname + '.s')
	asmbl = jz_101_assembler()
	asmbl.run(asm_code)
	write_secondary_output(asm_fname + '.d',asmbl)
	write_hex_file(asm_fname + '.h',asmbl)
